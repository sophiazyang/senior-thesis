...
-   The dataset contains data from Summer 2 2012 (Term `{r} min(combined_df$term_enrolled)`) to Spring 2025 (Term `{r} max(combined_df$term_enrolled)`).
...
term_lookup <- tribble(
  ~term_number, ~academic_term,     ~academic_year,
  0010,         "Prior Terms",      NA,
  0120,         "Summer 1 1984",    "1983-1984",
  0280,         "Spring 1992",      "1991-1992",
  0320,         "Fall 1992",        "1992-1993",
  0500,         "Fall 1995",        "1995-1996",
  0620,         "Fall 1997",        "1997-1998",
  0680,         "Fall 1998",        "1998-1999",
  0740,         "Fall 1999",        "1999-2000",
  0800,         "Fall 2000",        "2000-2001",
  0905,         "Summer 2 2002",    "2002-2003",
  0920,         "Fall 2002",        "2002-2003",
  0980,         "Fall 2003",        "2003-2004",
  1040,         "Fall 2004",        "2004-2005",
  1115,         "Fall 2005",        "2005-2006",
  1165,         "Summer 2 2006",    "2006-2007",
...
clean_term_df <- clean_df |>
  left_join(
    term_lookup |> rename(
      term_year_enrolled = academic_year,
      term_enrolled_new = academic_term
    ),
    by = c("term_enrolled" = "term_number")
  ) |>
  left_join(
    term_lookup |> rename(
      term_year_admit = academic_year,
      term_admit = academic_term
    ),
  ...
  mutate(
    term_enrolled = factor(term_enrolled_new,
                                levels = term_levels,
                                ordered = TRUE),
    term_admit = factor(term_admit,
                             levels = term_levels,
                             ordered = TRUE)
  ) |>
  select(-c(admit_term, term_enrolled_new))
```
...
new_vars_df <- clean_term_df |>
  ...
  group_by(term_enrolled, class_nbr_rev) |> # INACCURATE RN
...
# Not sure if will keep, some departments don't have BS
...
# Summer courses could be indicators of financial aid, pre-med, lack of internship, retaking a course, etc
summer_student <- new_vars_df |>
  group_by(masked_student_ID) |>
  summarise(
...
# What about eda of students who took summer vs who didn't
```
...
semGPA_df <- new_vars_df |>
  ...
  filter(
    str_starts(term_enrolled, "Spring") | 
      str_starts(term_enrolled, "Fall"),
...
# Number of students admitted prior to 2009-2010 academic year
final_df |>
  group_by(masked_student_ID) |>
  filter(term_year_admit <= "2009-2010") |>
  distinct(masked_student_ID) |>
  nrow()

# Number of students admitted prior to 2012-2013 academic year
final_df |>
  group_by(masked_student_ID) |>
  filter(term_year_admit <= "2012-2013") |>
  distinct(masked_student_ID) |>
  nrow()
...
# Number of students before cleaning
num_students <- final_df |>
  distinct(masked_student_ID) |>
  nrow()
num_students

# Cleaning, do this after GPA
final_clean_df <- final_df |>
  filter(!str_detect(division_of_course, "Other"))

# Number of observations after cleaning
nrow(final_clean_df)

# Cleaning, can do this after creating and factoring term_year_admit
...
# Number of students after cleaning
final_clean_df |>
  distinct(masked_student_ID) |>
  nrow()

# Cleaning, can do this after creating took_summer_classes
final_clean_df <- final_clean_df |>
  filter(!str_starts(term_enrolled, "Summer"))
...
num_rows - nrow(final_clean_df)

# Number of students remaining
final_students <- final_clean_df |>
  distinct(masked_student_ID) |>
  nrow()
final_students
# Total number of students removed
num_students - final_students
```

Dropping "Other" since hard to interpret

Course data starts in Summer 2012, only include students that began in academic year 2012-2013 or in 2009-2010 (spring 2012 graduates began in 2008-2009)
...
+-------------------------------------------+-----------------------+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| division_of_course                        | categorical           | Arts and Humanities, Social Sciences, Natural Sciences (see Trinity) + Engineering, Writing, and Other                                                                               |
+-------------------------------------------+-----------------------+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
+-------------------------------------------+-----------------------+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| is_SU                                     | binary                | response variable in model                                                                                                                                                           |
+-------------------------------------------+-----------------------+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
+-------------------------------------------+-----------------------+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| total_students_in_course                  | numerical             | Approximate total number of students in a course in a given semester (computed by counting how many observations during a given term_enrolled for a course_id, includes withdrawals) |
+-------------------------------------------+-----------------------+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
final_clean_df |>
  group_by(term_enrolled, division_of_course) |>
  summarise(
    n_students = n_distinct(masked_student_ID),
    .groups = "drop"
  ) |>
  ...
  labs(
    title = "Proportion of Students by Division Over Terms",
    x = "Term Enrolled",
...
final_clean_df |>
  ...
  scale_fill_viridis_d() +
  labs(
    title = "Proportion of S/U Courses by Division Over Terms",
    x = "Term Enrolled",
...
prevGPA_df |>
  ...
  ggplot(aes(x = term_enrolled, y = avg_semGPA)) +
  geom_line() +
  labs(
...
```{r}
model_timeperiod <- glm(is_SU ~ catalog_level + division_of_course +
                          class_component + student_acad_prog + has_BS + 
                          timeperiod + took_summer_courses, 
             data = final_clean_df, family = binomial)
summary(model_timeperiod)
```

```{r}
model_term <- glm(is_SU ~ term_enrolled + catalog_level + division_of_course +
                    class_component + student_acad_prog + has_BS +
                    took_summer_courses, 
             data = final_clean_df, family = binomial)
summary(model_term)
