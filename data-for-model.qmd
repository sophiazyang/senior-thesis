...
# Not using this...
underload_ids <- only8_sem_df |>
  filter(!str_starts(term_enrolled, "Summer")) |>
  group_by(masked_student_ID, term_enrolled) |>
  summarise(term_units = sum(max_units, na.rm = TRUE), .groups = "drop") |>
  ...
  summarise(
    underload = any(term_units < 4 &
                    !str_starts(term_enrolled, "Summer") &
                    semester_num < 8), # allow underloads last semester
    .groups = "drop"
  ) |>
  filter(underload) |>
  pull(masked_student_ID)

no_underload_df <- only8_sem_df |> 
  filter(!masked_student_ID %in% underload_ids)
```
...
saveRDS(final_clean_df, file = "data/modeling.rds")
```
