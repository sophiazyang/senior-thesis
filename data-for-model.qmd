...
# Define the grades to check
all_grades <- c("A+", "A", "A-", "B+", "B", "B-", 
                "C+", "C", "C-", "D+", "D", "D-", "F")

# Step 1: Find courses with no A-F grades
courses_no_grades <- gpa_df |>
  group_by(term_enrolled, course_ID) |>
  summarize(letter_grades = any(final_grade %in% all_grades), .groups = "drop") |>
  filter(!letter_grades) |>
  select(term_enrolled, course_ID)

# Number of courses with no A-F grades
nrow(courses_no_grades)

# Step 2: Drop them from the original dataset
graded_df <- gpa_df |>
  anti_join(courses_no_grades, by = c("term_enrolled", "course_ID"))
```

```{r}
#| label: remove-unknown-courses
...
```{r}
#| label: compute-props
# Imputing for students with at least 8 semesters and ignoring those who may have dropped out prior to 2019
students_missing_major <- valid_courses_df |>
...
# --- Step 2: Compute upper-level (>199) course counts (fixed) ---
...
# --- Step 3: Get the "true" divisions from declared major flags ---
student_flags <- students_with_major |>
  select(masked_student_ID, student_group, is_art_humanity:is_natural_sci) |>
  pivot_longer(
    cols = is_art_humanity:is_natural_sci,
    names_to = "division_flag",
    values_to = "is_division"
  ...
  filter(division %in% division_cols) |>
  select(masked_student_ID, student_group, division, is_division) |>
  distinct()

# --- Step 4: Merge proportions with known flags ---
check_table <- student_div_props |>
  left_join(student_flags, by = c("masked_student_ID", "student_group", "division"))

# --- Step 5: Function to compute classification error at a given threshold ---
compute_error <- function(p_thresh) {
  check_table |>
...
# --- Step 6: Apply to all thresholds ---
errors_by_p <- map_dfr(p_vals, compute_error)

# --- Step 7: Find best threshold per student group and division (tie-safe) ---
best_p <- errors_by_p |>
  group_by(student_group, division) |>
  ...
  slice(1) |>
  ungroup() |>
  select(student_group, division, best_p = p, min_error = error_rate)

# --- Step 8: Apply best thresholds to classify students ---
student_flags_estimated <- check_table |>
  left_join(best_p, by = c("student_group", "division")) |>
  mutate(estimated_flag = prop_division > best_p)

# --- Step 9: Compute accuracy by student group and division ---
...
ggplot(accuracy_by_group, aes(x = student_group, y = best_p, color = division)) +
  geom_point(size = 3) +
  geom_line(aes(group = division), linewidth = 1) +
  scale_color_brewer(palette = "Set2") +
  labs(
    title = "Best p-values by Student Group and Division",
    x = "Student Group",
    y = "Best p-value",
    color = "Division"
  ) +
...
ggplot(accuracy_by_group, aes(x = student_group, y = min_error, color = division)) +
  geom_point(size = 3) +
  geom_line(aes(group = division), linewidth = 1) +
  scale_color_brewer(palette = "Set1") +
  labs(
    title = "Minimum Error by Student Group and Division",
    x = "Student Group",
    y = "Minimum Error",
    color = "Division"
  ) +
...
model_df <- final_with_estimates |>
  ...
  filter(actual_load != "underload") # cannot S/U if underloading
