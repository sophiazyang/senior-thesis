...
\[TODO, coming soon via email announcement from Joan\]
...
\[TODO: add clarification on which students and which classes can be taken S/U\]
...
var_select |> kable(
  digits = 3,
  caption = "AIC and BIC metrics excluding select variables. Inclusion of all variables improves model fit."
) |> 
...
After addressing major modeling hurdles, I used the selected features and hyperparameters while adding a random effect on student ID using `lme4::lmer()`. The random effect serves to address violations of independence due to the repetition of students across the dataset. For instance, voluntary S/U courses are capped at 4.0 credits to count towards graduation requirements and students can only request voluntary for a single 1.0 credit course.

\[TODO SUBJECT TO TIME CONSTRAINTS\] In order to answer more nuanced questions, I also fit a pair of generalized linear mixed models on a subset of my data. Specifically, I focus on the difference between the 2018-2019 academic year (right before pandemic) and the 2024-2025 academic year (furthest after pandemic). Naturally, this pair of models drops the timeperiod variable. For these models no additional hyperparameter tuning or variable selection is performed.
...
![Calibration plot from running logistic regression with SMOTENC without random effects. The dashed red line represents a perfect model while the solid blue line represents my model.](smotenc/glm_calibration_plot.png){height="40%"}
...
![Calibration plot from running logistic regression with random effects on students (without SMOTENC). The dashed red line represents a perfect model while the solid blue line represents my model.](smoteless/glmm_calibration_plot.png){height="40%"}
...
dharma_stats |> 
  kable(
  digits = 3,
  caption = "DHARMa results from Kolmogorov-Smirnov (KS) and dispersion tests",
...
all_metrics |>
  filter(!Metric %in% c("AIC", "BIC", "ICC_Conditional", "Brier Score")) |> # TODO add back brier
  filter(!Section=="Classification Metrics") |>
  mutate(
    Section = factor(Section,
           levels = c("Model Fit Statistics", 
  ...
  kable(
    digits = 3,
    caption = "Model statistics from model with random effect (using performance and yardstick)"
  ) |> 
  kable_paper("striped", full_width = F) |>
  pack_rows("Model Fit", 1, 3) |>
  pack_rows("Predictive Metrics", 4, 5) |>
  kable_styling(latex_options = "HOLD_position")
```
...
## \[SUBJECT TO TIME CONSTRAINTS\] Comparing 2018-2019 vs 2024-2025
