...
**TO DO FIX THE COLORS/WEIGHT BY ENROLLMENT??**
...
### Logistic Regression without Student Effect
...
Table 3 provides a more comprehensive view of model diagnostics. For model fit, there does not seem to be signs of significant under (\<1) or overdispersion (\>1). In terms of classification metrics, there is a relatively high ROC (Receiver Operating Characteristics) AUC (Area Under the Curve) of 0.825 which indicates good discrimination between positive and negative outcomes. A random model would score 0.5 while a perfect model would score 1.0. Given one positive and one negative case, the model will on average rank the positive one higher 82.5% of the time. The area under the PR (Precision-Recall) curve is an abyssal 0.086. However, due to only 2% of my data being of the positive class, the baseline PR AUC is 0.02; my model performs better than a random model. Finally, Brier Score is a supplement that measures how far predictions are from observations. A Brier Score of 0.764 is considered fairly high and is a negative signal of performance, related to the low PR AUC and specificity. Due to the relatively low number of On the otherhand, specificity is high as most observations are positive. F1 score serves as a balance between specificity and sensitivity....
...
all_metrics <- readRDS("smotenc/glm_all_metrics.rds")
all_metrics |> 
  filter(!Metric %in% c("AIC", "BIC")) |>
  mutate(
    Section = factor(Section,
           levels = c("Model Fit Statistics", 
                "Overall Predictive Metrics", "Classification Metrics"))
  ) |>
  arrange(Section) |>
  dplyr::select(-Section) |>
  kable(
    digits = 3,
    caption = "Model Diagnostic Values from running SMOTENC without random effect"
  ) |> 
  kable_paper("striped", full_width = F) |>
  pack_rows("Model Fit", 1, 2) |>
  pack_rows("Classification Metrics", 3, 5) |>
  pack_rows("Predictive Metrics", 6, 9) |>
  kable_styling(latex_options = "HOLD_position")
```
...
![Calibration shows under predicting](smoteless/glmm_calibration_plot.png){height="40%"}

![text](smoteless/glmm_qq_plot.png){height="40%"}

![text](smoteless/glmm_roc_pr_plot.png){height="40%"}

![text](smoteless/glmm_pearson_plot.png){height="40%"}

![text](smoteless/glmm_binned_plot.png){height="40%"}

![text](smoteless/glmm_dharma_plot.png){height="40%"}
...
In terms of actual modeling approach, I make assumptions in order to fit the logistic regression. Notably, I assume that predictors are linear with the log odds of the outcome (choice of voluntary S/U). Based on ridge regression's inclusion of both course units and the categorical load as well as the clear patterns in my residual plots, there is evidence that there may be nonlinearity. Additionally, the synthetic data from SMOTE may not adequately match the actual distribution of the minority class, nor did I verify the distribution of these synthetic data.
...
class component, tune decision threshold with other hyperparm, tune neighbors, use of model better suited for imbalance (random forest), glmmTMB, instead of SMOTE use ... interaction terms
