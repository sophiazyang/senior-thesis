...
\[SUBJECT TO TIME CONSTRAINTS\] In order to answer more nuanced questions, I also fit a pair of generalized linear mixed models on a subset of my data. Specifically, I focus on the difference between the 2018-2019 academic year (right before pandemic) and the 2024-2025 academic year (furthest after pandemic). Naturally, this pair of models drops the timeperiod variable. For these models no additional hyperparameter tuning or variable selection is performed.
...
all_metrics |>
  filter(!Metric %in% c("AIC", "BIC", "ICC_Conditional")) |>
  ...
  pack_rows("Predictive Metrics", 4, 6) |>
  kable_styling(latex_options = "HOLD_position")
```
...
```{r, echo=FALSE}
all_metrics <- readRDS("smoteless/glmm_coeff.rds")
all_metrics |> 
...
As a supplement to check model fit, I also plot ROC and PR curves (Figure 12) and compute the area under the curves (AUC) and Brier Score (Table 5). The ROC and its associated AUC support my model (PR AUC = 0.823) as it clearly performs better than a random model would (PR AUC = 0.5). While the PR curve and its associated AUC are very low (PR AUC = 0.083), the baseline for PR AUC is relative to the positive class's prevalence. A completely random model would have a baseline performance of 0.02. Thus, while a low bar, my model performs better than random—despite never predicting a positive. Finally, Brier Score is the average squared distance between the predicted and actual probabilities. Like PR AUC, its baseline is relative to class prevalence. For ......

Due to the lack of positive predictions, I omit the typical analysis of precision, recall, specificity, and F1 score.

## Significant Covariates

From my model estimates (Table 6) I find several covariates to be statistically significant (p-value \< 0.05). Starting from the top, as expected, the intercept is very small as voluntary S/U is extremely uncommon

\[log odds thing first then interpret\]

prev_semGPA higher the less likely to S/U

take summer courses (true?) more likely to S/U

signif diff with is_natural_sci -\> less likely to S/U

more overloads (so far?) -\> less likely to S/U (maybe from those overloads alrdy did their S/Us OR maybe its part of num of sem, less likely to S/U later in acad career?)

acad load not signif

overload -\> less likely to S/U

postpan -\> more likely to S/U

pan -\> more likely to S/U

acad lvl 2 -\> more likely

acad lvl 3 -\> more likely

acad lvl 4 -\> more likely

\*! lvl 1 baseline is 1-99... so 5 = 400-499

cata lvl 2 more likely su

cata lvl 5 less likely su

nat sci -\> more likely

ss -\> more likely

not signif interaction

NS taking art more likely su

SS taking SS less likely su
