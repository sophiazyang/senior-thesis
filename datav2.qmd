...
# study abroad could be indicator of willingness to explore, 
# taking advantage of alternative grading systems, etc
studied_abroad <- combined_df |>
  group_by(masked_student_ID) |>
  summarise(
...
# WORK IN PROGRESS INACCURATE
# are graduate classes messing this up?
# did we do credits differently in the past

course_load_df <- with_gpa_df |>
...
# Cleaning, can do this after creating took_summer_classes and summer EDA
...
selected_years <- c("2009-2010", "2014-2015", "2019-2020", "2024-2025")
...
```{r}
model_timeperiod <- glm(is_SU ~ catalog_level + division +
                          student_acad_prog + 
...
```{r}
model_term <- glm(is_SU ~ term_enrolled + catalog_level + division +
                    student_acad_prog +
                    took_summer_courses, 
             data = final_clean_df, family = binomial)
summary(model_term)
...
Need to check to see if GPA and stuff correct

total_enrolled has some 0s???
