...
```{r}
# downsample + weights strategy
glmer_recipe_combined <- recipe(is_SU ~ ., data = train_df_small) |>
  update_role(masked_student_ID, new_role = "id") |>
  step_impute_median(all_numeric_predictors()) |>
  step_impute_mode(all_nominal_predictors()) |>
...
pos_weight_combined <- 5 
train_with_weights <- train_df_small |>
  mutate(
    case_weight = if_else(is_SU == "1", pos_weight_combined, 1.0)
  )

glmer_spec <- mixed_model(family = "binomial") |>
  set_engine("glmer") |>
  set_mode("classification")

glmer_workflow_combined <- workflow() |>
  add_recipe(glmer_recipe_combined) |>
  add_model(
    glmer_spec,
    formula = is_SU ~ . + (1 | masked_student_ID)
  )

glmer_fit_combined <- fit(
  glmer_workflow_combined,
  data = train_with_weights, # Use the data with the weight column
  case_weights = case_weight  # Pass the column name to fit()
)
```
