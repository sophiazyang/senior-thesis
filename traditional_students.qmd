...
imbalance_by_group <- df |>
  group_by(student_group) |>
  summarise(
    n_total = n(),
    n_true  = sum(is_SU, na.rm = TRUE),
    n_false = sum(!is_SU, na.rm = TRUE),
    prop_true = mean(is_SU, na.rm = TRUE)
  ) |>
  arrange(desc(prop_true))

# --- overall imbalance (across all groups) ---
overall_imbalance <- df |>
  summarise(
    n_total = n(),
    n_true  = sum(is_SU, na.rm = TRUE),
    n_false = sum(!is_SU, na.rm = TRUE),
