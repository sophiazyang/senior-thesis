...
### Testing actual_load vs load_status

```{r}
su_recipe_actload <- recipe(is_SU ~ actual_load + prev_semGPA + num_plans +
                      studied_away + took_summer_courses +
                      is_art_humanity + is_social_sci + is_natural_sci +
                      timeperiod + academic_level_bot +
                      catalog_level + division +
...
vars_actload <- selected_vars[!str_detect(selected_vars, "load_status")]
...
su_recipe_inter <- recipe(is_SU ~ load_status + prev_semGPA + num_plans +
                      studied_away + took_summer_courses +
                      is_art_humanity + is_social_sci + is_natural_sci +
                      timeperiod + academic_level_bot +
                      catalog_level + division +
...
subject_vars <- c("is_natural_sci", "is_art_humanity", "is_social_sci")
division_dummies <- c("division_Natural.Sciences", "division_Social.Sciences", "division_Engineering", "division_WRITING")
interaction_terms <- as.vector(outer(subject_vars, division_dummies, FUN = function(x,y) paste0(x, ":", y)))

all_terms <- c(selected_vars, interaction_terms)
glm_formula_actload <- as.formula(
    paste("is_SU ~", paste(all_terms, collapse = " + "))
  )
glm_fit_inter <- glm(
  glm_formula_actload,
...
test_results <- augment(glmer_fit, new_data = test_df_small) |>
  mutate(
