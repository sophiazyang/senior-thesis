...
train_df_weighted <- train_df_weighted %>%
...
test_df_weighted <- test_df_weighted %>%
  mutate(final_grade = recode(final_grade,
    "A+" = "A_plus",
    "A"  = "A",
    "A-" = "A_minus",
...
glmer_recipe <- recipe(is_SU ~ ., data = train_df_weighted) %>%
  update_role(weight, new_role = "case_weight") %>%
  ...
  step_mutate(
    ...
    prev_semGPA  = if_else(is.na(prev_semGPA), 4, prev_semGPA),
    did_indep_study = factor(did_indep_study),
    unknown = factor(is.na(is_art_humanity))
  ) %>%
  step_impute_median(all_numeric_predictors()) %>%
  step_impute_mode(all_nominal_predictors()) %>%
...
# 3. Keep only LASSO-selected columns + masked_student_ID + weight + outcome
glmer_data <- train_baked %>%
  select(all_of(selected_vars), masked_student_ID, weight, is_SU)
