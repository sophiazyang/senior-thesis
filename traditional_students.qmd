...
df <- df |>
  select(-c("term_enrolled", "course_ID", "estimated", "did_indep_study", "student_group", "year_enrolled", "max_units", "final_grade"))
...
#| label: preprocess
#| eval: false
su_recipe_lasso <- recipe(is_SU ~ load_status + term_units + actual_load + actual_units +
...
su_recipe_fixed <- recipe(is_SU ~ ., data = train_df) |>
  ...
  step_impute_median(all_numeric_predictors()) |>
  step_impute_mode(all_nominal_predictors()) |>
  step_dummy(all_nominal_predictors(), -has_role("ID"), one_hot = FALSE) |>
  step_normalize(all_numeric_predictors()) |>
  step_zv(all_predictors()) |>
  step_interact(
    terms = ~ is_natural_sci:starts_with("division_") +
...
su_recipe_nostudaway <- recipe(is_SU ~ load_status + term_units + actual_load + actual_units +
                      prev_semGPA + num_plans +
                      took_summer_courses +
                      is_art_humanity + is_social_sci + is_natural_sci +
                      timeperiod + academic_level_bot +
...
su_recipe_nosumm <- recipe(is_SU ~ load_status + term_units + actual_load + actual_units + 
                      prev_semGPA + num_plans +
                      studied_away +
                      is_art_humanity + is_social_sci + is_natural_sci +
                      timeperiod + academic_level_bot +
...
su_recipe_noplans <- recipe(is_SU ~ load_status + term_units + actual_load + actual_units +
                      prev_semGPA +
                      studied_away + took_summer_courses +
                      is_art_humanity + is_social_sci + is_natural_sci +
                      timeperiod + academic_level_bot +
...
su_recipe_noenroll <- recipe(is_SU ~ load_status + term_units + actual_load + actual_units + 
                      prev_semGPA + num_plans +
                      studied_away + took_summer_courses +
                      is_art_humanity + is_social_sci + is_natural_sci +
                      timeperiod + academic_level_bot +
...
```{r}
#| eval: false
# Model with interactions between student and course divisions
su_recipe_inter <- recipe(is_SU ~ load_status + term_units + actual_load + actual_units + 
                      prev_semGPA + num_plans +
                      studied_away + took_summer_courses +
                      is_art_humanity + is_social_sci + is_natural_sci +
                      timeperiod + academic_level_bot +
...
models <- list(
  ...
  Without_interaction = glm_fit_inter
)
...
saveRDS(model_table, "data/model_table.rds")
```

```{r}
model_table <- readRDS("data/model_table.rds")
model_table
```
...
# playing around...
df <- model.frame(glmer_fit)
df$pear <- residuals(glmer_fit, type = "pearson")
df$fit <- fitted(glmer_fit)
...
abline(h = 0, col = "red")
hist(df$pear, main = "Distribution of Pearson Residuals", xlab = "Pearson residuals")
qqnorm(df$pear); qqline(df$pear, col = "red")
ggplot(df, aes(x = fit, y = pear)) +
  geom_point(alpha = 0.3) +
  geom_smooth(se = FALSE, color = "blue") +
  geom_hline(yintercept = 0, color = "red") +
  labs(x = "Fitted values", y = "Pearson residuals",
       title = "Checking for nonlinearity or heteroskedasticity")
library(arm)
binnedplot(df$fit, df$pear, xlab = "Predicted probability", ylab = "Residuals",
           main = "Binned residual plot")
```

```{r}
# Evaluate performance on test
trained_recipe <- prep(glmer_recipe)
test_baked <- bake(trained_recipe, new_data = test_df_small)
...
pos_weight_combined <- 5.0 

train_with_weights <- train_df_small |>
  mutate(
    case_weight = if_else(as.character(is_SU) == "1", pos_weight_combined, 1.0)
  )

glmer_recipe2 <- recipe(is_SU ~ ., data = train_with_weights) |>
  update_role(masked_student_ID, new_role = "id") |>
  update_role(case_weight, new_role = "weight", bake = FALSE) |>
  step_impute_median(all_numeric_predictors()) |>
  step_impute_mode(all_nominal_predictors()) |>
  ...
  step_downsample(is_SU, under_ratio = 5) 

train_baked_full <- prep(glmer_recipe2) |> 
  bake(new_data = train_with_weights)
glmer_data_final <- train_baked_full |>
  dplyr::select(all_of(selected_vars), masked_student_ID, is_SU, case_weight) 
glmer_data_final$case_weight <- as.numeric(glmer_data_final$case_weight)
glm_formula <- as.formula(
  paste0("is_SU ~ ", paste(selected_vars, collapse = " + "), " + (1 | masked_student_ID)")
)

glmer_fit_combined <- glmer(
  glm_formula,
  data = glmer_data_final,
  family = binomial(link = "logit"),
  weights = glmer_data_final$case_weight, 
  control = glmerControl(optimizer = "bobyqa", calc.derivs = FALSE)
)

summary(glmer_fit_combined)

```

```{r}
# Model fit
check_overdispersion(glmer_fit_combined)

AIC(glmer_fit_combined)

BIC(glmer_fit_combined)

icc(glmer_fit_combined)

# Evaluate performance on test
trained_recipe2 <- prep(glmer_recipe2)
test_df_small$case_weight <- 1
test_baked2 <- bake(trained_recipe2, new_data = test_df_small)
test_baked2$.pred_1 <- predict(
  glmer_fit,
  newdata = test_baked2,
  type = "response",
  allow.new.levels = TRUE   # allows new students in test set
...
# ROC / AUC
roc_auc(test_results2, truth = is_SU, .pred_1, estimator = "binary", event_level = "second")

# Plot ROC
roc_df2 <- roc_curve(test_results2, truth = is_SU, .pred_1, event_level = "second")
...
# PR-AUC
pr_auc(
  test_results2, truth = is_SU, .pred_1,
  event_level = "second"
)

# Plot PR
pr_df2 <- pr_curve(test_results2, truth = is_SU, .pred_1, event_level = "second")
ggplot(pr_df2, aes(x = recall, y = precision)) +
  geom_line(color = "darkgreen", size = 1.2) +
  labs(
    title = "Precision-Recall Curve",
    x = "Recall (Sensitivity)",
...
# Confusion matrix
cm2 <- conf_mat(test_results2, truth = is_SU, estimate = pred_class)
cm2
...
ggplot(calibration_df2, aes(x = mean_pred, y = obs_rate)) +
  geom_point(size = 3, color = "blue") +
  geom_line(color = "blue") +
  # Add the "perfect calibration" line
  geom_abline(
