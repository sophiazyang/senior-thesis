...
class_division <- combined_df |>
  count(division) |>
  mutate(prop = n / sum(n)) |>
  arrange(n)
print(class_division, n = Inf)
```
...
clean_term_df <- clean_df |>
  ...
  filter(term_enrolled > "Summer 2 2012")

saveRDS(clean_term_df, file = "data/2-fall2012.rds")
```
...
# Drop transfer students and students with unknown matriculuation
timeperiod_df |>
  mutate(starts_with_TR = str_sub(student_group, 1, 2) == "TR") |>
  count(starts_with_TR)

admit_df <- timeperiod_df |>
  filter(str_sub(student_group, 1, 2) != "TR") |>
  mutate(
    student_group = str_replace(student_group, "^..", "20"),
    student_group = as.numeric(student_group)
  )

saveRDS(admit_df, file = "data/5-admission.rds")
```
...
saveRDS(plans_df, file = "data/6-numplans.rds")
```
...
underload_df <- plans_df |>
  group_by(masked_student_ID, term_enrolled) |>
  mutate(term_units = sum(max_units, na.rm = TRUE)) |>   # total units that term
  group_by(masked_student_ID) |>
  mutate(
  underload = as.integer(
    any(term_units < 4 & !str_starts(term_enrolled, "Summer"))
  )) |>
  ungroup()
  
academ_df <- underload_df |>
...
course_load_df <- academ_df |>
  ...
  mutate(
    ...
    early_grad = if_else(max(semester_num, na.rm = TRUE) < 8, 1L, 0L)
  ) |>
  ungroup()
...
saveRDS(numbered_df, file = "data/7-courseload.rds")
```
...
saveRDS(away_df, file = "data/8-abroad.rds")
```
...
saveRDS(no_summer_df, file = "data/9-no-summer.rds")
```

```{r}
#| label: missing-majors

proportion_first_major <- no_summer_df |>
  group_by(student_group, masked_student_ID) |>
  summarise(
    has_first_major = mean(!is.na(first_major_descr)),
    .groups = "drop"
  ) |>
  group_by(student_group) |>
  summarise(
    prop_major = mean(has_first_major),
    n_students = n(),
    .groups = "drop"
  )
print(proportion_first_major, n = Inf)

fy2010_df <- no_summer_df |>
  filter(student_group > 2009)

saveRDS(fy2010_df, file = "data/10-fy2010.rds")
```
...
# dropping 204 students with program 2 or unknown IDM
student_divisions |>
  filter(tbd == 1L) |>
  summarise(n = n())

divisions_df <- fy2010_df |>
  left_join(student_divisions, by = "masked_student_ID") |> 
  filter(!(tbd == 1L) | is.na(tbd))

saveRDS(divisions_df, file = "data/11-major-div.rds")
```
...
saveRDS(gpa_df, file = "data/12-gpa.rds")

class_components2 <- gpa_df |>
  count(class_component) |>
  mutate(prop = n / sum(n)) |>
  arrange(n)
print(class_components2, n = Inf)
```

```{r}
#| label: indep-study

course_counts <- gpa_df |>
  group_by(term_enrolled, course_ID) |>
  summarise(
    num_students = n_distinct(masked_student_ID),
    .groups = "drop"
  )

# Step 2: join back into your main pipeline
final_clean_df <- gpa_df |>
  left_join(course_counts, by = c("term_enrolled" = "term_enrolled", 
                                  "course_ID" = "course_ID"))


final_clean_df <- final_clean_df |>
  mutate(
    did_indep_study = class_component == "IND"
  )

saveRDS(final_clean_df, file = "data/13-final.rds")
```
...
columns are students who have at least one major that falls into that division

rows are classes that belong to which division

cells are the probability a student who has major in that division takes a class in the division (student lvl)

```{r}
# Step 1: put majors in long format
major_long <- final_clean_df |>
  select(masked_student_ID, division,
         is_art_humanity, is_social_sci, is_natural_sci, is_engr, tbd) |>
  pivot_longer(
    cols = c(is_art_humanity, is_social_sci, is_natural_sci, is_engr, tbd),
    names_to = "major_division",
    values_to = "has_major"
  )

# Step 2: expand to all student × division combos
all_divisions <- unique(final_clean_df$division)

expanded <- major_long |>
  distinct(masked_student_ID, major_division, has_major) |>
  crossing(division = all_divisions) |>
  left_join(
    major_long |> distinct(masked_student_ID, division) |> mutate(took_course = 1),
    by = c("masked_student_ID", "division")
  ) |>
  mutate(took_course = if_else(is.na(took_course), 0L, took_course))

# Step 3: counts for students WITH the major
counts <- expanded |>
  filter(has_major == 1) |>   # only students who have that major
  group_by(division, major_division) |>
  summarise(
    n_total = n(),
    n_took  = sum(took_course),
    .groups = "drop"
  )

# Step 4: compute probability + Wilson CI
probs <- counts |>
  rowwise() |>
  mutate(
    ci = list(binom.confint(n_took, n_total, methods = "wilson"))
  ) |>
  unnest(ci) |>
  transmute(
    division,
    major_division,
    display = sprintf("%.3f [%.3f, %.3f]", mean, lower, upper)
  ) |>
  pivot_wider(names_from = major_division, values_from = display)

# ---- Step 2: compute last row (percent of students who have each major) ----
total_students <- expanded |>
  group_by(major_division) |>
  summarise(
    pct_students = mean(has_major) * 100,
    .groups = "drop"
  ) |>
  transmute(major_division, display = sprintf("%.1f%%", pct_students)) |>
  pivot_wider(names_from = major_division, values_from = display) |>
  mutate(division = "Pct Ppl")

# ---- Step 3: compute last column (percent of courses in each division) ----
total_courses <- final_clean_df |>
  group_by(division) |>
  summarise(
    pct_courses = n() / nrow(final_clean_df) * 100,
    .groups = "drop"
  ) |>
  transmute(division, Total = sprintf("%.1f%%", pct_courses))

# ---- Step 4: merge totals into table ----
final_table <- probs |>
  left_join(total_courses, by = "division") |>
  bind_rows(total_students)

# Example abbreviation mapping for majors
abbr_map <- c(
  "is_art_humanity" = "ART",
  "is_social_sci"   = "SS",
  "is_natural_sci"  = "NS",
...
final_table_abbr <- final_table |>
  rename_with(~ abbr_map[.x], .cols = names(abbr_map))

# Example for row names / course divisions
division_map <- c(
  "Arts & Humanities" = "ART",
  "Social Sciences"   = "SS",
  "Natural Sciences"  = "NS",
...
final_table_abbr <- final_table_abbr |>
  mutate(division = recode(division, !!!division_map))

n_cols <- ncol(final_table_abbr)
n_rows <- nrow(final_table_abbr)

final_table_abbr |>
  kable(
    format = "latex",
    booktabs = TRUE,
    caption = "Student-Level Probabilities by Major and Course Division",
    align = c("l", rep("c", n_cols - 1))
  ) |>
  kable_styling(latex_options = c("striped", "hold_position")) |>
  row_spec(n_rows, bold = TRUE) |>
  column_spec(n_cols, bold = TRUE)

```

```{r}
# Percent of SU courses that are U
final_clean_df |>
  filter(is_SU == 1) |>
  summarise(
    pct_U = mean(final_grade == "U", na.rm = TRUE)
  )

# Percent of non-SU courses that are D-F
grades_DF <- c("D-", "D", "D+", "F")
final_clean_df |>
  filter(is_SU == 0) |>
  summarise(
    pct_DF = mean(final_grade %in% grades_DF, na.rm = TRUE)
  )
```
...
# --- Parameters ---
division_cols <- c("Arts & Humanities", "Social Sciences", "Natural Sciences") # exclude Engineering
x_vals <- 1:14  # candidate thresholds

# --- Step 1: Student-level flags ---
student_flags <- students_with_major |>
  select(masked_student_ID, is_art_humanity:is_engr) |>
  pivot_longer(
    cols = is_art_humanity:is_engr,
...
students_missing_major <- final_clean_df |>
  ...
  filter(student_acad_prog == "Trinity") |>
  filter(max_units >= 1)
