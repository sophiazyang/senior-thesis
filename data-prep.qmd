...
saveRDS(away_df, file = "data/7-abroad.rds")
```
...
saveRDS(no_summer_df, file = "data/8-no-summer.rds")
```
...
course_load_df <- academ_df |>
  ...
  mutate(
    # Increment semester counter, note represents which semester not how many
    semester_num = row_number(),
...
saveRDS(numbered_df, file = "data/9-courseload.rds")
```

```{r}
#| label: missing-majors

proportion_first_major <- numbered_df |>
...
# format final_table for display
final_table_abbr |>
  kable(
  ...
  column_spec(ncol(final_table), bold = TRUE)
```
