...
```{r}
result$pear <- residuals(glmer_fit, type = "deviance")
result$fit <- fitted(glmer_fit)
...
glmm_deviance <- ggplot(result, aes(x = fit, y = pear)) +
  geom_point(alpha = 0.3) +
  geom_smooth(se = FALSE, color = "blue") +
  geom_hline(yintercept = 0, color = "red") +
  labs(x = "Fitted values", y = "Deviance residuals",
       title = "Residuals vs Fitted Values")
ggsave("smoteless/glmm_deviance_plot.png", plot = glmm_deviance, dpi = 300)
...
out_disp       <- testDispersion(sim)
ks_p       <- out_ks$p.value
disp_p     <- out_disp$p.value

dharma_stats <- tibble(
  test = c("KS uniformity", "Dispersion"),
  statistic = c(out_ks$statistic, out_disp$statistic),
  p_value = c(out_ks$p.value, out_disp$p.value),
)
saveRDS(dharma_stats, "smoteless/dharma_stats.rds")
```
