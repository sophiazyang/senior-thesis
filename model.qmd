...
# ROC / AUC
rocauc <- roc_auc(test_results, truth = is_SU, .pred_1, estimator = "binary", event_level = "second")
...
glmm_coeff <- kable(model_tidy, digits = 3, caption = "GLMM Fixed Effects") |>
  kable_styling(full_width = FALSE) |>
  column_spec(1, width = "10em")
saveRDS(glmm_coeff, "data/glmm_coeff.rds")

result <- readRDS("data/model_resids.rds")
result$pear <- residuals(glmer_fit, type = "pearson")
result$fit <- fitted(glmer_fit)

png("figures/glmm_qq_plot.png", width = 6*300, height = 4*300, res = 300) 
...
ggsave("figures/glmm_pearson_plot.png", plot = glmm_pearson, dpi = 300)

png("figures/glmm_binned_plot.png", width = 6*300, height = 4*300, res = 300) 
...
png("figures/glmm_dharma_plot.png", width = 6*300, height = 4*300, res = 300) 
plot(sim)
dev.off()
```
...
all_metrics
saveRDS(all_metrics, "data/glmm_metrics.rds")
...
roc <- ggplot(roc_df, aes(x = 1 - specificity, y = sensitivity)) +
  ...
  theme_minimal()
ggsave("figures/glmm_roc_plot.png", roc)
...
pr <- ggplot(pr_df, aes(x = recall, y = precision)) +
  ...
  theme_minimal()
ggsave("figures/glmm_pr_plot.png", pr)
...
ggsave("figures/glmm_calibration_plot.png", calibration)
```
...
model_tidy <- tidy(glmer_fit, effects = "fixed", conf.int = TRUE)
saveRDS(model_tidy, "smotenc/model_tidy.rds")
...
glmm_coeff <- kable(model_tidy, digits = 3, caption = "GLMM Fixed Effects") |>
  kable_styling(full_width = FALSE) |>
  column_spec(1, width = "10em")
saveRDS(glmm_coeff, "smotenc/glmm_coeff.rds")
...
all_metrics
saveRDS(all_metrics, "smotenc/glmm_metrics.rds")
...
model_tidy <- tidy(glmer_fit, effects = "fixed", conf.int = TRUE)
saveRDS(model_tidy, "smoteless/model_tidy.rds")
...
glmm_coeff <- kable(model_tidy, digits = 3, caption = "GLMM Fixed Effects") |>
  kable_styling(full_width = FALSE) |>
  column_spec(1, width = "10em")
saveRDS(glmm_coeff, "smoteless/glmm_coeff.rds")
...
all_metrics
saveRDS(all_metrics, "smoteless/glmm_metrics.rds")
...
roc <- ggplot(roc_df, aes(x = 1 - specificity, y = sensitivity)) +
  ...
  theme_minimal()
ggsave("smoteless/glmm_roc_plot.png", roc)
...
pr <- ggplot(pr_df, aes(x = recall, y = precision)) +
  ...
  theme_minimal()
ggsave("smoteless/glmm_pr_plot.png", pr)
