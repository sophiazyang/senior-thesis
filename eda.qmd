...
df_away <- readRDS("data/7-courseload.rds")
df_summer <- readRDS("data/8-abroad.rds")
```
...
ggplot(df_course_counts, aes(x = term_enrolled, y = enrollment_count)) +
  ...
  theme_minimal() +
  labs(
    title = "Course Size Distribution by Term",
...
```{r}
ggplot(df, aes(x = term_enrolled, y = total_enrolled)) +
  geom_boxplot(fill = "#1f77b4") +
  theme_minimal() +
  labs(
    title = "Course Size Distribution by Term",
...
```{r}
df_course_counts |>
  filter(enrollment_count > 500)

df |>
  filter(course_ID == "012627") |>
  select(division, catalog_level, location, max_units)
```
...
df_div_summary <- df |>
  group_by(term_enrolled, division) |>
  summarise(
    mean_size = mean(total_enrolled),
    sd_size   = sd(total_enrolled),
    n_classes = n(),
    .groups = "drop"
  ) |>
  mutate(
...
ggplot(df_SU_total, aes(x = factor(total_overloads), y = total_SU)) +
  geom_boxplot(alpha = 0.7, outlier.alpha = 0.3) +
  theme_minimal() +
  labs(
    title = "Total S/U Courses vs Total Overloads",
    x = "Total Number of Overloads",
    y = "Total S/U Courses"
  )
```
